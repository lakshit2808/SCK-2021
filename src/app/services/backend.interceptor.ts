...
export class BackendInterceptor implements HttpInterceptor {
  ...
  //  This will also skip the interception if Write-Object header is present
    if (request.headers.has(WriteObject)) {
      ...
      const updatedRequest = request.clone({
        setParams: {
          consumer_key : environment.writableKey.customer_key,
          consumer_secret: environment.writableKey.customer_secret
        },
        headers
      });
  ...
  //  If both the headers are not present, process normally
    const modifiedRequest = request.clone({
      setParams: {
        consumer_key: environment.readOnlyKeys.customer_key,
        consumer_secret: environment.readOnlyKeys.customer_secret
      }
    });
